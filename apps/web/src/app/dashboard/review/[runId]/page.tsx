...
interface PlaylistResult {
  id: string; name: string; dimension: string; taxonomyValue: string;
  trackCount: number; spotifyPlaylistId: string | null; spotifyUrl: string | null;
}
...
export default function ReviewPage() {
  ...
  useEffect(() => {
    if (run?.status !== 'CREATING_PLAYLISTS') return;
    const interval = setInterval(() => {
      void (async () => {
        const res = await fetch(`${API}/api/classify/${runId}`, { headers: authH() });
        if (!res.ok) return;
  ...
  function removeArrayTag(id: string, field: 'genres' | 'moods' | 'occasions', value: string) {
    ...
    if (!track) return;
    const updated: ClassifiedTrack = { ...track, [field]: track[field].filter((v: string) => v !== value) };
  ...
  function addArrayTag(id: string, field: 'genres' | 'moods' | 'occasions', value: string) {
    const track = groupTracksRef.current.find(ct => ct.id === id);
    if (!track) return;
    const existing: string[] = track[field];
  ...
  async function handlePush() {
    ...
    const res = await fetch(`${API}/api/classify/${runId}/playlists`, { method: 'POST', headers: authH() });
    if (res.ok) {
      setRun(r => r ? { ...r, status: 'CREATING_PLAYLISTS' } : r);
    } else {
      setPushState('error');
    }
  }
  ...
  const totalGroups = (Object.keys(DIMS) as DimKey[]).reduce((n, dim) => n + (summary[dim]?.length ?? 0), 0);
  const isApproved   = approveState === 'done' || ['APPROVED', 'CREATING_PLAYLISTS', 'DONE'].includes(run.status);
  const isPushing    = run.status === 'CREATING_PLAYLISTS';
  const isDone       = pushState === 'done' || run.status === 'DONE';
  const createdCount = playlists.filter(p => p.spotifyPlaylistId !== null).length;
    ...
    <div className="min-h-screen bg-spotify-black flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
          ...
          {isApproved && !isPushing && !isDone && (
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              >
                <Play className="w-4 h-4" /> Push to Spotify
              </button>
            </div>
          )}
          ...
          {/* Pushing in progress */}
          {isPushing && (
            <div className="flex items-center gap-2 text-spotify-text-muted text-sm flex-shrink-0">
              <Loader2 className="w-4 h-4 animate-spin text-spotify-green" />
              <span className="hidden sm:inline">Pushing to Spotify…</span>
            </div>
          )}
          ...
          {isDone && (
            <div className="flex items-center gap-2 text-spotify-green text-sm font-semibold flex-shrink-0">
              ...
              <span className="hidden sm:inline">{createdCount} playlist{createdCount !== 1 ? 's' : ''} on Spotify</span>
            </div>
          )}

          {pushState === 'error' && (
            <span className="text-xs text-red-400 flex-shrink-0">Push failed — retry</span>
          )}
        </div>
      </header>
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-6 py-6 flex gap-6">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          {/* Playlists created — results grid */}
          {!selectedGroup && isDone && (
            <div className="flex-1 overflow-y-auto">
              <div className="mb-5">
                <h2 className="font-bold text-base mb-0.5">Playlists on Spotify</h2>
                <p className="text-xs text-spotify-text-muted">
                  {createdCount} playlist{createdCount !== 1 ? 's' : ''} created · open Spotify to start listening
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {playlists.map(p => {
                  ...
                  const DimIcon = dimMeta.Icon;
                  return (
                    <div key={p.id} className="flex items-center gap-3 bg-spotify-surface rounded-xl px-4 py-3 group">
                      <div className={`w-10 h-10 ${dimMeta.panelBg} rounded-lg flex items-center justify-center flex-shrink-0`}>
                        <DimIcon className={`w-4.5 h-4.5 ${dimMeta.accent}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate">{p.name}</p>
                        <p className="text-xs text-spotify-text-muted">{p.trackCount} tracks · {p.dimension}</p>
                      </div>
                      {p.spotifyUrl ? (
                        <a
                          href={p.spotifyUrl} target="_blank" rel="noopener noreferrer"
                          className="flex-shrink-0 flex items-center gap-1 text-[11px] text-spotify-green font-semibold hover:text-white transition-colors"
                        >
